...
// SERIAL CONVOLUTION
// --------------------------------------------------------------------------------------
void serialConvolution(const Image& input, Image& output, const Kernel& kernel) {
        ...
        for (int x = 0; x < cols; x++) {
            for (int c = 0; c < channels; c++) {
                if (c == 3) { // Preserve alpha channel
                    output.data[(y * cols + x) * channels + c] = input.data[(y * cols + x) * channels + c];
                    continue;
                }
                ...
                output.data[(y * cols + x) * channels + c] = static_cast<unsigned char>(sum);
            }
        }
    }
}
...
// --------------------------------------------------------------------------------------
void parallelConvolution(const Image& input, Image& output, const Kernel& kernel, int threads) {
    int rows = input.height;
    int cols = input.width;
    int kSize = kernel.size;
    int halfK = kSize / 2;
...
// --------------------------------------------------------------------------------------
void tiledParallelConvolution(const Image& input, Image& output, const Kernel& kernel, int threads, int TILE_SIZE = 64) {
    int rows = input.height;
    int cols = input.width;
    int kSize = kernel.size;
    int halfK = kSize / 2;
    ...
    // We collapse outer tile loops. Threads grab whole 64x64 chunks.
    // This strongly enforces L1 cache usage.
    #pragma omp parallel for collapse(2) schedule(dynamic)
    for (int ty = 0; ty < rows; ty += TILE_SIZE) {
        for (int tx = 0; tx < cols; tx += TILE_SIZE) {
            
            // Standard convolution strictly within this tile bounds
...
void parallelInversion(const Image& input, Image& output, int threads) {
    ...
    for (int i = 0; i < total_bytes; i++) {
        // Skip alpha channel logic for simplicity assuming 3 channel RGB for our tests
...
void avx2ParallelInversion(const Image& input, Image& output, int threads) {
    ...
    for (int b = 0; b < blocks; b++) {
        ...
        __m256i pixels = _mm256_loadu_si256((__m256i*)&input.data[i]); // Load 32 bytes unaligned
        __m256i inv = _mm256_sub_epi8(vec_255, pixels); // 255 - pixel
        _mm256_storeu_si256((__m256i*)&output.data[i], inv); // Store 32 bytes
    }
...
void parallelGrayscale(const Image& input, Image& output, int threads) {
    ...
    for (int i = 0; i < total_pixels; i++) {
        ...
        unsigned char gray_val = static_cast<unsigned char>(max(0.0f, min(255.0f, gray)));

        output.data[i * channels + 0] = gray_val;
        output.data[i * channels + 1] = gray_val;
        output.data[i * channels + 2] = gray_val;
        if (channels == 4) output.data[i * channels + 3] = input.data[i * channels + 3];
    }
}
...
void parallelSepia(const Image& input, Image& output, int threads) {
    ...
    for (int i = 0; i < total_pixels; i++) {
        ...
        output.data[i * channels + 0] = static_cast<unsigned char>(max(0.0f, min(255.0f, tr)));
        output.data[i * channels + 1] = static_cast<unsigned char>(max(0.0f, min(255.0f, tg)));
        output.data[i * channels + 2] = static_cast<unsigned char>(max(0.0f, min(255.0f, tb)));
        if (channels == 4) output.data[i * channels + 3] = input.data[i * channels + 3];
    }
}

// Fast Sobel Magnitude combining X and Y
void computeSobelMagnitude(const Image& sobelX, const Image& sobelY, Image& output, int threads) {
    ...
    for (int i = 0; i < total_pixels; i++) {
        for (int c = 0; c < channels; c++) {
            ...
            output.data[i * channels + c] = static_cast<unsigned char>(min(255.0f, mag));
        }
    }
}
...
int main(int argc, char** argv) {
    ...
    unsigned char* img_data = stbi_load(imagePath.c_str(), &width, &height, &channels, 0);
    if (!img_data) {
    ...
    csvFile << "filter_name,threads,serial_time,parallel_time,speedup,efficiency\n";
    ...
    auto runBenchmark = [&](const string& filterName, auto serialFunc, auto parallelFunc, Image& final_out) {
        ...
        cout << "Serial Time: " << fixed << setprecision(4) << time_serial << " s" << endl;
        ...
        for (int t : thread_counts) {
            ...
            cout << "Threads: " << setw(2) << t 
                 << " | Time: " << fixed << setprecision(4) << time_parallel << " s"
                 << " | Speedup: " << fixed << setprecision(2) << speedup << "x"
                 << " | Eff: " << fixed << setprecision(1) << (efficiency * 100) << "%" << endl;

            csvFile << filterName << "," << t << "," << time_serial << "," << time_parallel << "," << speedup << "," << efficiency << "\n";
        }
    };
    ...
    runBenchmark("Gaussian Blur (Scalar)", 
        [&](Image& out) { serialConvolution(image, out, gaussianK); },
        [&](Image& out, int t) { parallelConvolution(image, out, gaussianK, t); },
        out_gaussian
    );
    stbi_write_jpg("output_gaussian.jpg", width, height, channels, out_gaussian.data, 90);

    runBenchmark("Gaussian Blur (Tiled 64x64)", 
        [&](Image& out) { tiledParallelConvolution(image, out, gaussianK, 1, 64); },
        [&](Image& out, int t) { tiledParallelConvolution(image, out, gaussianK, t, 64); },
        out_tiled
    );
    // Tiled output is visually identical, no need to save again

    // BENCHMARK 2: Image Inversion (Scalar vs AVX2)
    runBenchmark("Inversion (Scalar)", 
        [&](Image& out) { parallelInversion(image, out, 1); }, 
        [&](Image& out, int t) { parallelInversion(image, out, t); },
        out_inv
    );

    runBenchmark("Inversion (AVX2 SIMD)", 
        [&](Image& out) { avx2ParallelInversion(image, out, 1); }, 
        [&](Image& out, int t) { avx2ParallelInversion(image, out, t); },
        out_inv
    );
    stbi_write_jpg("output_inversion.jpg", width, height, channels, out_inv.data, 90);

    // BENCHMARK 3: Grayscale
    runBenchmark("Grayscale", 
        [&](Image& out) { parallelGrayscale(image, out, 1); }, // simulate serial with 1 thread
        [&](Image& out, int t) { parallelGrayscale(image, out, t); },
        out_gray
    );
    stbi_write_jpg("output_grayscale.jpg", width, height, channels, out_gray.data, 90);

    // BENCHMARK 3: Sepia
    runBenchmark("Sepia tone", 
        [&](Image& out) { parallelSepia(image, out, 1); }, 
        [&](Image& out, int t) { parallelSepia(image, out, t); },
        out_sepia
    );
    stbi_write_jpg("output_sepia.jpg", width, height, channels, out_sepia.data, 90);

    // BENCHMARK 4: Sharpen
    runBenchmark("Sharpening", 
        [&](Image& out) { serialConvolution(image, out, sharpenK); },
        [&](Image& out, int t) { parallelConvolution(image, out, sharpenK, t); },
        out_sharpen
    );
    stbi_write_jpg("output_sharpen.jpg", width, height, channels, out_sharpen.data, 90);
    ...
    for (int t : thread_counts) {
        ...
        csvFile << "Sobel Edges," << t << "," << time_serial << "," << time_parallel << "," << speedup << "," << efficiency << "\n";
    }
    stbi_write_jpg("output_sobel.jpg", width, height, channels, out_sobelMag.data, 90);
